...
func LookupWriter(w io.WriteCloser, opts WriterOpts) (zio.WriteCloser, error) {
	if opts.Format == "" {
		opts.Format = "tzng"
	}
	switch opts.Format {
...
func lookupReader(r io.Reader, zctx *zson.Context, opts ReaderOpts) (zio.Reader, error) {
	switch opts.Format {
	case "csv":
