...
func formatAny(arena *zed.Arena, val zed.Value, inContainer bool) string {
	...
	case *zed.TypeNamed:
		return formatAny(arena, arena.NewValue(t.Type, val.Bytes()), inContainer)
	case *zed.TypeOfBool:
		if val.Bool() {
			return "T"
		}
	...
	case *zed.TypeEnum:
		return formatAny(arena, arena.NewValue(zed.TypeUint64, val.Bytes()), false)
...
func formatArray(arena *zed.Arena, t *zed.TypeArray, zv zcode.Bytes) string {
	...
	for !it.Done() {
		...
		if val := it.Next(); val == nil {
			...
			b.WriteString(formatAny(arena, arena.NewValue(t.Type, val), true))
		}
	}
	return b.String()
}
...
func formatMap(arena *zed.Arena, t *zed.TypeMap, zv zcode.Bytes) string {
	...
	b.WriteByte('[')
	for !it.Done() {
		b.WriteString(formatAny(arena, arena.NewValue(t.KeyType, it.Next()), true))
		b.WriteString(formatAny(arena, arena.NewValue(t.ValType, it.Next()), true))
	}
	b.WriteByte(']')
	return b.String()
}
...
func formatRecord(arena *zed.Arena, t *zed.TypeRecord, zv zcode.Bytes) string {
	...
	for _, f := range t.Fields {
		...
		if val := it.Next(); val == nil {
			...
			b.WriteString(formatAny(arena, arena.NewValue(f.Type, val), false))
		}
	}
	return b.String()
}
...
func formatSet(arena *zed.Arena, t *zed.TypeSet, zv zcode.Bytes) string {
	...
	for !it.Done() {
		...
		b.WriteString(formatAny(arena, arena.NewValue(t.Type, it.Next()), true))
	}
	return b.String()
}
...
func formatUnion(arena *zed.Arena, t *zed.TypeUnion, zv zcode.Bytes) string {
	...
	return s + formatAny(arena, arena.NewValue(typ, iv), false)
}
