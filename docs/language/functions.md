...
`from_hex` decodes hex encoded string `s` into a byte sequence.
...
```mdtest-command
echo  '{id:0x0dfc90519b60f362e84a3fdddd9b9e63e1fb90d1(=ksuid.KSUID)}' \
  | zq -z 'id := ksuid(id)' -
```
...
`to_base64` encodes byte sequence `b` into a base 64 encoded string.
