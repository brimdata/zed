...
[`this`](../language/overview/dataflow-model.md#the-special-value-this) and you say:
```mdtest-command
...
[shorthand for](../language/overview/dataflow-model.md#implied-operators) `search 2` so the command
```mdtest-command
echo '1 2 3' | zq -z 2 -
```
...
[Record expressions](../language/overview/expressions.md#record-expressions)
...
is [union types](../language/overview/expressions.md#union-values).
...
...
```
Note that we used [string interpolation](../language/overview/expressions.md#string-interpolation)
...
create this in Zed with a ["lateral subquery"](../language/overview/lateral-subqueries.md).
...
...
```
Note that the syntax `=> ( ... )` defines a [lateral scope](../language/overview/lateral-subqueries.md#lateral-scope) where any Zed subquery can
...
[record literal](../language/overview/expressions.md#record-expressions) with the desired value:
```mdtest-command dir=docs/tutorials
