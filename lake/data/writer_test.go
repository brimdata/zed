...
func TestDataReaderWriterVector(t *testing.T) {
	...
	defer a.KeepAlive()
	require.NoError(t, w.Write(zson.MustParseValue(a, "{a:1,b:4}")))
	require.NoError(t, w.Write(zson.MustParseValue(a, "{a:2,b:5}")))
	require.NoError(t, w.Write(zson.MustParseValue(a, "{a:3,b:6}")))
