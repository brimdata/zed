...
type rangeFinder func(context.Context, *data.Object) (seekindex.Range, error)

func newRangeFinder(pool *Pool, snap commits.View, f zbuf.Filter) (rangeFinder, error) {
	trunc, err := f.AsObjectTruncatedFilter(pool.Layout.Order, pool.Layout.Primary())
	if err != nil {
		return nil, err
	}
	idx := index.NewFilter(pool.engine, pool.IndexPath, f)
	pkf := f.(*kernel.Filter).PoolKeyFilter(pool.Layout.Primary())
	...
	return func(ctx context.Context, o *data.Object) (seekindex.Range, error) {
		...
		if indexSpan != nil || trunc != nil && trunc.Eval(span.First(), span.Last()) {
			...
			return seekindex.Lookup(r, pkf, indexSpan, &o.Last, o.Count, o.Size, pool.Layout.Order)
		}
		return rg, nil
	}, nil
}
