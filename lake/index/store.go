...
func (s *Store) load(ctx context.Context) error {
	...
	for {
		...
		var rule interface{}
		if err := s.unmarshaler.Unmarshal(rec.Value, &rule); err != nil {
			return err
		}
...
func (s *Store) Names(ctx context.Context, key string) ([]string, error) {
...
func (s *Store) Add(ctx context.Context, rule Rule) error {
	...
	for attempts := 0; attempts < maxRetries; attempts++ {
		...
		if _, ok := s.table[id]; ok {
			return fmt.Errorf("index rule %q in set %q already exists", id, name)
		}
