...
func (r *remote) Query(ctx context.Context, zctx *zed.Context, head *lakeparse.Commitish, src string, srcfiles ...string) (zio.ReadCloser, error) {
	q, err := r.QueryWithControl(ctx, zctx, head, src, srcfiles...)
	if err != nil {
		return nil, err
	}
...
func (r *remote) QueryWithControl(ctx context.Context, zctx *zed.Context, head *lakeparse.Commitish, src string, srcfiles ...string) (zbuf.ProgressReadCloser, error) {
	res, err := r.conn.Query(ctx, head, src, srcfiles...)
	if err != nil {
		return nil, err
	}
	q, err := queryio.NewQuery(zctx, res.Body), nil
	if err != nil {
		return nil, err
	}
