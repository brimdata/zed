...
func ctx() proc.Parent {
	c := &proc.Context{TypeContext: resolver.NewContext()}
	return c.NewParent(nil)
}

// Test that illegal cut operations fail at compile time with a
// reasonable error message.
func testNonAdjacentFields(t *testing.T, zql string) {
	_, err := proctest.CompileTestProc(zql, ctx())
...
func TestDuplicateFieldErrors(t *testing.T) {
	...
	_, err := proctest.CompileTestProc("cut a,ab", ctx())
	require.NoError(t, err)

	_, err = proctest.CompileTestProc("cut ab,a", ctx())
	require.NoError(t, err)
}
