...
func firstOf(d *zq.Descriptor, which zeek.Type) string {
...
func firstNot(d *zq.Descriptor, which zeek.Type) string {
	for _, col := range d.Type.Columns {
		if !zeek.SameType(col.Type, which) {
			return col.Name
		}
...
func guessSortField(rec *zq.Record) string {
	d := rec.Descriptor
...
func (s *Sort) Pull() (zq.Batch, error) {
	for {
		batch, err := s.Get()
		if err != nil {
			return nil, err
...
func (s *Sort) consume(batch zq.Batch) {
...
func (s *Sort) sort() zq.Batch {
	...
	sortWithDir := func(a, b *zq.Record) int {
