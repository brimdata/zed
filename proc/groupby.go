...
type GroupByAggregator struct {
	...
	logger       *zap.Logger
	limit        int
	sortFn       expr.ValueSortFn // to compare primary keys for early key output
	maxKey       *zng.Value
	inputSortDir int
}
...
func NewGroupByAggregator(c *Context, params GroupByParams) *GroupByAggregator {
	...
	var sortFn expr.ValueSortFn
	if len(params.keys) > 0 && params.inputSortDir != 0 {
		s := expr.NewValueSortFn(true)
		if params.inputSortDir < 0 {
			sortFn = func(a, b zng.Value) int { return s(b, a) }
		} else {
			sortFn = s
		}
	}
...
func (g *GroupByAggregator) records(eof bool) ([]*zng.Record, error) {
	...
	for _, k := range keys {
		row := g.table[k]
		primKey, err := getPrimaryKey(row)
		if err != nil {
			return nil, err
		}
		if !eof && g.sortFn(primKey, *g.maxKey) >= 0 {
			continue
		}
...
func getPrimaryKey(row *GroupByRow) (zng.Value, error) {
	iter := zcode.Iter(row.keyvals)
	for {
		b, container, err := iter.Next()
		if err != nil {
			return zng.Value{}, err
		}
		if !container || b == nil {
			return zng.Value{row.keycols[0].Type, b}, nil
		}
		iter = zcode.Iter(b)
	}
}
