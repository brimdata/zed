...
func (p *Proc) run() {
	defer func() {
		close(p.resultCh)
		if p.agg.manager != nil {
			p.agg.manager.Cleanup()
		}
	}()
	for {
...
func (a *Aggregator) spillTable(eof bool) error {
	...
	if a.manager == nil {
		a.manager, err = runmanager.NewManager(a.keysCompare)
		if err != nil {
			return err
		}
	}
	...
	if err := a.manager.CreateRun(recs); err != nil {
		return err
	}
...
func (a *Aggregator) nextResultFromSpills() (*zng.Record, error) {
	...
	for {
		...
		if _, err := a.manager.Read(); err != nil {
			return nil, err
		}
	}
	if firstRec == nil {
