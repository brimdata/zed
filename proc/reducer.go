...
func (r *Reducer) output() *zq.Array {
	rec := r.columns.Result(r.Context.Resolver)
	return zq.NewArray([]*zq.Record{rec}, nano.NewSpanTs(r.MinTs, r.MaxTs))
}

func (r *Reducer) Pull() (zq.Batch, error) {
	start := time.Now()
	for {
		batch, err := r.Get()
		if err != nil {
...
func (r *Reducer) consume(rec *zq.Record) {
	r.n++
