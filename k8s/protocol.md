...
(In the story below, when we inroduce a new type of message in our protocol, we name it in all caps, e.g. REDIRECT. The protocol message discussion is speculative, it is not based on an existing protocol like HTTP.)
...
7. In addition to the sequence of types and records in ZNG, there is additional information that the manager and worker processes must exchange: the manager must initiate the conversation with the worker by sending a serialization of an AST of procs that represents the subset of the manager's AST that is being delegated to the worker. The serialized proc must contain a reference to the source of data which the worker will process -- we will call this combination a JOB message, which includes an AST + datasource. Simple examples of data sources are files or S3 objects. More complex data sources can be other workers that have been recruited by the same manager, to work on other parts of the manager's AST (more on worker-to-worker streams later.)
...
3. At that point, the manager will recruit two (or more) workers to process the group-by. The group-by key space is split based on a hash function (e.g. xxhash, FarmHash or some other uniformly distributed non-cryptographic hash function.) 
...
7. At some point, a worker may realize it is running out of key space, and make a decision to split its work, in the same way as the manager did previously. It will then send a SPLIT message to the manager. When the manager receives the first SPLIT request from a group-by worker, it can anticipate that the other workers will soon run out of key space, since the work is being uniformly distruted between all the group-by worker by a hash function. So, in reponse, the manager recruits a number of new group-by workers equal to the number of existing group-by workers.
When the new workers are available, the manager will send a SPLIT with the new "routing table" to the existing group-by workers. Each worker will forward "half" of its existing group-by keys to a new worker (its newly desigated helper, as defined in the routing table). The existing workers will start to buffer records that should be processed by the new workers until the new workers have received the existing keys and have "come online".
8. When the new workers are ready (have received the work-in-progress group-by keys) they notify the manager. The manager then broadcasts a new "routing table" to all the filter workers.
9. The filter workers are sending their work directly to the group-by workers. When they are have completed work they send a DONE response to the manager. When all filter workers have sent a DONE response to the manager, it broadcasts a "FINISH" message to all of the group-by workers.
...
BTW, if a worker requests a SPLIT but does not get the opportunity to split, then it should spill "half" its key space to disk, and buffer "half" of its incoming work for processing on a second pass.
...
The discussion of group-by above assumes there is only one "map" function: a crytographic hash, and only one "reduce" function: merging sorted keys from multiple streams. This is not as general as MapReduce, but works well for the example we considered.

In addition, the discussion assumes continuing work (in the form of ZNG records) is routed directly to peer workers, rather than being passed down through a tree, as in MapReduce. This both reduces the number of "hops" between processes, and is also more amenable to streaming work continously.

In the example above, the number of group-by workers can be adjusted dynamically, in response to to memory requirements. (The dynamic scaling is similar to Steve's suggestion for te continous shuffle algorithm.) This feels more "elastic" than MapReduce, but there might be variations on MapReduce that do similar dynamic scaling.

## Implmenting this as an extension to the ZNG protocol

Most of the inter-process traffic in our cluster of managers and workers will be data in ZNG format. To that, we have added several new message types. From our discussion above, we have:
* JOB has a serialized AST of procs and a datasouce to work on. It is sent from a manager to a recruited worker.
* DONE is sent from a worker to its manager when the JOB is complete. DONE is sent from a manager to a worker when no more JOBs are forthcoming.

These two message types handle parallelizable work. 
More message types are required for the distributed group-by case:
* SPLIT is sent from a worker process to it manager when it "runs out" of key space (or possibly hits another theshold, such as an inablity to keep up with the input stream)
* REDIRECT includes a routing table: traffic splitting rules for a set of host:port. It is sent from a manager to its workers to update a set of workers on where to send their work.
* FINISH is sent by a manager to a set of group-by workers to tell them to prepare to stream their work (in sorted order) back to the manager.

All these messages should coexist with the binary ZNG record (# lines and data lines) that will make up the vast majority of the traffic in the cluster. Since ZNG is most of the data, and is optimized for its purpose, we do not want to wrap it in another protocol.

So, it may be convenient to extend ZNG with new types of records that would include the ones above. The details of this are TBD. This extension of ZNG would only be used in the distributed processing cases above. There is probably no need to persist ZNG files that contain these extensions. They are defined only for coordination of work among cluster workers and would not have a clear meaning in a static file, AFAICT.
