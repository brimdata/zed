...
bin/$(ZEEKPATH):
	...
	@curl -L -o bin/$(ZEEKPATH).zip \
		https://github.com/brimsec/zeek/releases/download/$(ZEEKTAG)/zeek-$(ZEEKTAG).$$(go env GOOS)-$(GOARCH).zip
...
bin/$(SURICATAPATH):
	...
	curl -L -o bin/$(SURICATAPATH).zip \
		https://github.com/brimsec/build-suricata/releases/download/$(SURICATATAG)/suricata-$(SURICATATAG).$$(go env GOOS)-$(GOARCH).zip
