...
* The introduction of Zed lakes for data storage, which include powerful
  Git-like branching. See the [Zed lake README](docs/lake/README.md)
  for details.
...
* Make the [`fuse`](docs/language/operators.md#fuse) operator work on nested records (#2052)
...
* Fix an issue where [`cut`](docs/language/operators.md#cut) to the root would exit if the referenced field was missing from a record (#2121)
* Fix an issue where [`put`](docs/language/operators.md#put) to the root would panic on a non-record field (#2136)
...
* Extend [`join`](docs/language/operators.md#join) to support `inner` (now the default), `left`, and `right` variations (#2210)
...
* Add a reference [shaper for Zeek data](docs/zeek/Shaping-Zeek-NDJSON.md) (#2300, #2368, #2448, #2489, #2601)
...
* Add the [Zed lake design](docs/lake/README.md) README document (#2500, #2569, #2595, #2781, #2940, #3014, #3034, #3035)
...
* Require `on` in [`join`](docs/language/operators.md#join) syntax (#2698)
...
* Add [API docs](docs/lake/service-api.md) for the Zed lake service (#2679)
* Support `from file` in Zed language in `zq`, which is particularly useful with [`join`](docs/language/operators.md#example-1---inner-join) (#2753)
...
* Move and extend the [Zeek interoperability docs](docs/zeek/README.md) (#2770, #2782, #2830)
...
* Add Zed language documentation for the [`join` operator](docs/language/operators.md#join) (#2836)
...
* Field assignments in [`join`](docs/language/operators.md#join) now behave like [`cut`](docs/language/operators.md#cut) instead of [`pick`](docs/language/operators.md#pick) (#2868)
* Add more background/context to Zed top-level language [README](docs/language/README.md) (#2866 #2878, #2901)
* Unify `from`, `split`, and `switch` syntax to the forms shown [here](docs/language#directed-acyclic-flow-graphs) (#2871, #2896)
* Shapers can now cast values of the `null` type to any type (e.g., arrays or records) (#2882)
* Fix an issue where [`join`](docs/language/operators.md#join) was failing to match on values of comparable types (e.g., `string` and `bstring`) (#2880, #2884)
* Shapers can now cast a value to a `union` type (#2881)
* Introduce alternate `switch` syntax (#2888, #3004)
* When [`fuse`](docs/language/operators.md#fuse) encounters a field with the same name but different types, it now creates one field of `union` type rather than separate, uniquely-named fields (#2885, #2886)
* Fix an issue where [`fuse`](docs/language/operators.md#fuse) would consume too much memory when fusing many types (#2897, #2899)
* Emphasize in the [`sort`](docs/language/operators.md#sort) documentation that its output can be non-deterministic in the absence of an explicit field list (#2902)
...
* The [`put`](docs/language/operators.md#put) operator keyword is now optional (e.g., can write `x:=1` instead of `put x:=1`) (#2967, #2986, #3043)
* Fix an issue where a [`put`](docs/language/operators.md#put) on a nested record with an alias triggered a panic (#2990)
...
* Update the reference Zed shaper for Zeek ([shaper](docs/zeek/shaper.zed), [docs](docs/zeek/Shaping-Zeek-NDJSON.md)) to reflect changes in Zeek release v4.1.0 (#3021)
...
* zq: Fix an issue with [`rename`](docs/language/operators.md#rename) where a subsequent `count()` would return no results (#2046)
...
* zq: Add support in [`put`](docs/language/operators.md#put) for assigning to `.` and to nested fields (#2018)
...
* zson: Update [Zeek Interoperability doc](docs/zeek/Data-Type-Compatibility.md) to include current ZSON syntax (#1956)
* zq: Ensure the output from the [`fuse`](docs/language/operators.md#fuse) operator is deterministic (#1958)
...
* zq/zqd: Invoke [`fuse`](docs/language/operators.md#fuse) automatically when CSV output is requested (#1908)
* zq: Fix an issue where [`fuse`](docs/language/operators.md#fuse) was not preserving record order (#1909)
...
* zql: Add a new [`drop`](docs/language/operators.md#drop) operator, which replaces `cut -c` (#1773)
* zql: Add a new [`pick`](docs/language/operators.md#pick) operator, which acts like a stricter [`cut`](docs/language/operators.md#cut) (#1773, #1788)
...
* zql: Add support for [aggregation-less group by](docs/language/grouping.md#example-1-1) (#1615, #1623)
...
* zql: Add a [docs example](docs/language/grouping.md#example-4) showing `by` grouping with non-present fields (#1703)

## v0.23.0
* zql: Add `week` as a unit for [time grouping with `every`](docs/language/grouping.md#time-grouping---every) (#1374)
* zq: Fix an issue where a `null` value in a [JSON type definition](docs/zeek/README.md) caused a failure without an error message (#1377)
...
* zq: Allow the [`fuse` operator](docs/language/operators.md#fuse) to spill-to-disk to avoid memory limitations (#1355, #1402)
* zq: No longer require `_path` as a first column in a [JSON type definition](docs/zeek/README.md) (#1370)
* zql: Improve ZQL docs for [aggregate functions](docs/language/aggregate-functions.md) and [grouping](docs/language/grouping.md) (#1385)
...
* zq: Add a [JSON type definition](docs/zeek/README.md) for alert events in [Suricata EVE logs](https://suricata.readthedocs.io/en/suricata-5.0.2/output/eve/eve-json-output.html) (#1400)
...
* zql: Add [`where` filtering](docs/language/aggregate-functions.md#where-filtering) for use with aggregate functions (#1490, #1481, #1533)
* zql: Add [`union()`](docs/language/aggregate-functions.md#union) aggregate function (#1493, #1534)
* zql: Add [`collect()`](docs/language/aggregate-functions.md#collect) aggregate function (#1496, #1534)
* zql: Add [`and()`](docs/language/aggregate-functions.md#and) and [`or()`](docs/language/aggregate-functions.md#or) aggregate functions (#1497, #1534)
...
* zql: Add a new [`rename` operator](docs/language/operators.md#rename) to rename fields in a record (#998, #1038)
...
* zng: Revise [Zeek compatibility](docs/zeek/Data-Type-Compatibility.md) doc (#919)
* zql: Clarify [`cut` operator documentation](docs/language/operators.md#cut) (#924)
