## v0.30.0
* Revise Zed language to unify search and expressions (#2072, #2152, #2252, #2304, #2294)
...
* Add a `switch` operator (#2087, #2364)
* Add constants and type literals to the Zed language (#2181)
* The `-I` option in `zq` is now used for file includes (and allows multiple files), with `-z` now used for compact ZSON output (#2180, #2208)
...
* Add the `sample` operator (#2200, #2211, #2623)
* Make `.`/`this` an implicit argument to `shape()` (#2199)
...
* Add canonical Zed and the `summarize` operator (#2217, #2378, #2408, #2698)
* Add support for casting the `duration` type (#2194)
* Extend [`join`](https://github.com/brimdata/zed/tree/main/docs/language/operators#join) to support `inner` (now default), `left`, and `right` (#2210)
...
* Add a reference [shaper for Zeek data](https://github.com/brimdata/zed/blob/main/zeek/Shaping-Zeek-NDJSON.md) (#2300, #2368, #2448, #2489, #2601)
...
* Fix an issue where the `switch` operator was creating duplicate records (#2318)
* Add support for reading comma-separated value (CSV) files (#2317, #2858)
* Fix an issue where reading a Zeek TSV log line would cause a panic if it contained too few fields (#2325)
* Add a `shape` operator, which is useful for cleaning up CSV inptus (#2327)
* Add a `default` case for the `switch` operator (#2336)
...
* Create the `zed` command with sub-commands like `query` and `api`, but shortcut commands (e.g., `zq`, `zapi`) also remain (#2450, #2465, #2466, #2463, #2624, #2620)
* Rename `ZAR_ROOT` environment variable to `ZED_LAKE_ROOT` (#2469)
* Revise the top-level [Zed README](https://github.com/brimdata/zed/blob/main/README.md) to reflect reorganization of the repo and new/changed tools (#2461)
* Remove the `-P` flag from `zq` in favor of using `from` in the Zed language (2491)
* Add casting to the `net` data type (#2493, #2496)
* `zq` now reads its inputs seqeuntially rather than the prior merged behavior (#2492)
...
* Add the [Zed lake design](https://github.com/brimdata/zed/blob/main/docs/lake/design.md) document (#2500, #2569, #2595, #2781)
...
* Revert the "auto-fuse" behavior originally added in #1908 (#2522)
* Add support for Git-style Zed lakes (#2548, #2556, #2562, #2563, #2564, #2566, #2571, #2577, #2580, #2616, #2613, #2735, #2763, #2805, #2808, #2811, #2816, #2860, #2861)
...
* Fix an issue where ZNG marshal of unexported struct fields caused a panic (#2589)
...
* `this` can now be used to reference the current top-level record (formerly `.`, which may be deprecated) (#2650)
* Rework dataflow model and compiler optimizations (#2669)
* Add initial `explode` operator that can break values from complex fields into separate records (#2673)
* Fix an issue where including a particular `time` field in a shaper script caused errors shaping other fields (#2685)
* Silently disard duplicate fields when reading NDJSON records, which works around [Suricata bug 4016](https://redmine.openinfosecfoundation.org/issues/4106) (#2691)
* Fix an issue where ZSON type values were output without parentheses (#2700)
* Swallow single-backslash-escaped `/` when reading NDJSON, which addresses a default Suricata behavior (#2697)
* Improve the error message shown when no Zed lake root is specified (#2701, #2739)
* Require `on` in [`join`](https://github.com/brimdata/zed/tree/main/docs/language/operators#join) syntax (#2698)
...
* Fix an issue where a shaper created a corrupt time value from an invalid timestamp rather than rejecting it (#2705)
* Simplify keyword search by requiring `:=` for assignment, `==` for comparison, and using `matches for regex match and glob match (#2692, #2744, #2773)
* Allow reading of data from `http://` and `https://` targets (#2723, #2732)
* Support for arbitrary pool keys in Zed lakes (#2729, #2752)
* Add [API docs](https://github.com/brimdata/zed/blob/main/docs/lake/service-api.md) for the Zed lake service (#2679)
* Support `from file` in Zed language in `zq`, which is particularly useful with [`join`](https://github.com/brimdata/zed/tree/main/docs/language/operators#example-1---inner-join) (#2753)
* Fix an issue where certain data could be queried successfully via `zq` but not if loaded into a Zed lake pool (#2755)
* Revise [Python client](https://github.com/brimdata/zed/tree/main/python) docs to show double quotes during `pip` install, since Windows needs that (#2758)
...
* Move and extend the [Zeek interoperability docs](https://github.com/brimdata/zed/blob/main/zeek/README.md) (#2770, #2782, #2830)
* Create endpoints in the Zed lake service API that correspond to underlying Zed lake operations, and expose them via `zapi` commands` (#2741, #2774, #2786, #2775, #2794, #2795, #2796)
...
* Add Zed language documentation for the [`join` operator](https://github.com/brimdata/zed/tree/main/docs/language/operators#join) (#2836)
* Fix an issue where reading a ZNG with more than 222 tyoe definitions triggered a `zng type ID out of range` error (#2847)
...
* Have the [Python client](https://github.com/brimdata/zed/tree/main/python) use the `/query` endpoint for the Zed lake (#2869)
* Minimize the amount of context shown when reporting parse errors (#2864)
...
* zql: Add a [docs example](docs/language/grouping#example-4) showing `by` grouping with non-present fields (#1703)
...
* zql: Add [`where` filtering](docs/language/aggregate-functions#where-filtering) for use with aggregate functions (#1490, #1481, #1533)
* zql: Add [`union()`](docs/language/aggregate-functions#union) aggregate function (#1493, #1534)
* zql: Add [`collect()`](docs/language/aggregate-functions#collect) aggregate function (#1496, #1534)
* zql: Add [`and()`](docs/language/aggregate-functions#and) and [`or()`](docs/language/aggregate-functions#or) aggregate functions (#1497, #1534)
...
* zql: [Document the `**` operator](docs/language/search-syntax#wildcard-field-names) for type-sepcific searches that look within nested records (#1337)
