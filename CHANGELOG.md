...
* Make the [`fuse`](docs/language/operators#fuse) operator work on nested records (#2052)
...
* Fix an issue where [`cut`](docs/language/operators#cut) to the root would exit if the referenced field was missing from a record (#2121)
* Fix an issue where [`put`](docs/language/operators#put) to the root would panic on a non-record field (#2136)
...
* Extend [`join`](docs/language/operators#join) to support `inner` (now the default), `left`, and `right` variations (#2210)
...
* Require `on` in [`join`](docs/language/operators#join) syntax (#2698)
...
* Support `from file` in Zed language in `zq`, which is particularly useful with [`join`](docs/language/operators#example-1---inner-join) (#2753)
...
* Add Zed language documentation for the [`join` operator](docs/language/operators#join) (#2836)
...
* Field assignments in [`join`](docs/language/operators/README.md#join) now behave like [`cut`](docs/language/operators/README.md#cut) instead of [`pick`](docs/language/operators/README.md#pick) (#2868)
...
* Fix an issue where [`join`](docs/language/operators/README.md#join) was failing to match on values of comparable types (e.g., `string` and `bstring`) (#2880, #2884)
* Shapers can now cast a value to a `union` type (#2881)
* Introduce alternate `switch` syntax (#2888, #3004)
* When [`fuse`](docs/language/operators#fuse) encounters a field with the same name but different types, it now creates one field of `union` type rather than separate, uniquely-named fields (#2885, #2886)
* Fix an issue where [`fuse`](docs/language/operators#fuse) would consume too much memory when fusing many types (#2897, #2899)
* Emphasize in the [`sort`](docs/language/operators#sort) documentation that its output can be non-deterministic in the absence of an explicit field list (#2902)
...
* The [`put`](docs/language/operators#put) operator keyword is now optional (e.g., can write `x:=1` instead of `put x:=1`) (#2967, #2986, #3043)
* Fix an issue where a [`put`](docs/language/operators#put) on a nested record with an alias triggered a panic (#2990)
