...
func FuzzQuery(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, b []byte) {
		...
		defer arena.Unref()
		types := fuzz.GenTypes(bytesReader, arena.Zctx(), 3)
		values := fuzz.GenValues(bytesReader, arena, types)
		...
		fuzz.WriteZNG(t, arena, values, &zngBuf)
		resultZNG := fuzz.RunQueryZNG(t, &zngBuf, querySource)

		var vngBuf bytes.Buffer
		fuzz.WriteVNG(t, arena, values, &vngBuf)
