...
func (c *Connection) SetAuthStore(store *auth0.Store) error {
	if store != nil {
		c.auth = store
		tokens, err := c.auth.LakeTokens(c.hostURL)
	...
	for i := 0; ; i++ {
		...
		if res.StatusCode < 200 || res.StatusCode > 299 {
			...
			if i == 0 && res.StatusCode == 401 && err.(*ErrorResponse).Err.Error() == "invalid token" {
				if err = c.refreshAuthToken(req.ctx); err == nil {
					continue
				}
			}
		}
...
func (c *Connection) refreshAuthToken(ctx context.Context) error {
	...
	refreshed, err := auth0.RefreshToken(ctx, tokens.Refresh, *method.Auth0)
	if err != nil {
		return err
	}
	c.SetAuthToken(refreshed.Access)
	c.auth.SetLakeTokens(c.hostURL, refreshed)
	return nil
}
