...
func (n *NetworkOf) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	id := args[0].Type().ID()
	if id != zed.IDIP {
		return arena.WrapError("network_of: not an IP", args[0])
	}
	...
	if len(args) == 1 {
		switch {
		case !ip.Is4():
			return arena.WrapError("network_of: not an IPv4 address", args[0])
		case ip.As4()[0] < 0x80:
			bits = 8
		case ip.As4()[0] < 0xc0:
			bits = 16
		...
		case id == zed.IDIP:
			...
			if netip.PrefixFrom(mask, bits).Masked().Addr() != mask {
				return arena.WrapError("network_of: mask is non-contiguous", args[1])
			}
...
func (c *CIDRMatch) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	if maskVal.Type().ID() != zed.IDNet {
		return ectx.Arena().WrapError("cidr_match: not a net", maskVal)
	}
