...
func (r *Regexp) Call(ectx expr.Context, args []zed.Value) zed.Value {
	if !args[0].IsString() {
		return ectx.Arena().WrapError("regexp: string required for first arg", args[0])
	}
	...
	if r.err != nil {
		return ectx.Arena().NewErrorf("regexp: %s", r.err)
	}
	if !args[1].IsString() {
		return ectx.Arena().WrapError("regexp: string required for second arg", args[1])
	}
	r.builder.Reset()
	...
	return ectx.Arena().NewValue(r.typ, r.builder.Bytes())
}
...
func (r *RegexpReplace) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	for i := range args {
		if !args[i].IsString() {
			return ectx.Arena().WrapError("regexp_replace: string arg required", args[i])
		}
	}
	if sVal.IsNull() {
		return zed.Null
	...
	if reVal.IsNull() || newVal.IsNull() {
		return ectx.Arena().NewErrorf("regexp_replace: 2nd and 3rd args cannot be null")
	}
	...
	if r.err != nil {
		return ectx.Arena().NewErrorf("regexp_replace: %s", r.err)
	}
