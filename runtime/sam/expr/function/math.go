...
func (a *Abs) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	return ectx.Arena().WrapError("abs: not a number", val)
}
...
func (c *Ceil) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	return ectx.Arena().WrapError("ceil: not a number", val)
}
...
func (f *Floor) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	return ectx.Arena().WrapError("floor: not a number", val)
}
...
func (l *Log) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	if !ok {
		return ectx.Arena().WrapError("log: not a number", args[0])
	}
	if x <= 0 {
		return ectx.Arena().WrapError("log: illegal argument", args[0])
	}
...
func (r *reducer) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	case zed.IsUnsigned(id):
		...
		for _, val := range args[1:] {
			...
			if !ok {
				return ectx.Arena().WrapError(r.name+": not a number", val)
			}
	...
	return ectx.Arena().WrapError(r.name+": not a number", val0)
}
...
func (r *Round) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	return ectx.Arena().WrapError("round: not a number", val)
}
...
func (p *Pow) Call(ectx expr.Context, args []zed.Value) zed.Value {
	x, ok := coerce.ToFloat(args[0])
	if !ok {
		return ectx.Arena().WrapError("pow: not a number", args[0])
	}
	y, ok := coerce.ToFloat(args[1])
	if !ok {
		return ectx.Arena().WrapError("pow: not a number", args[1])
	}
...
func (s *Sqrt) Call(ectx expr.Context, args []zed.Value) zed.Value {
	x, ok := coerce.ToFloat(args[0])
	if !ok {
		return ectx.Arena().WrapError("sqrt: not a number", args[0])
	}
