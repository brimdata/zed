...
func (r *Replace) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	for i := range args {
		if !args[i].IsString() {
			return ectx.Arena().WrapError("replace: string arg required", args[i])
		}
	}
	if sVal.IsNull() {
		return zed.Null
	...
	if oldVal.IsNull() || newVal.IsNull() {
		return ectx.Arena().NewErrorf("replace: an input arg is null")
	}
...
func (r *RuneLen) Call(ectx expr.Context, args []zed.Value) zed.Value {
	val := args[0]
	if !val.IsString() {
		return ectx.Arena().WrapError("rune_len: string arg required", val)
	}
	if val.IsNull() {
...
func (t *ToLower) Call(ectx expr.Context, args []zed.Value) zed.Value {
	val := args[0]
	if !val.IsString() {
		return ectx.Arena().WrapError("lower: string arg required", val)
	}
	if val.IsNull() {
...
func (t *ToUpper) Call(ectx expr.Context, args []zed.Value) zed.Value {
	val := args[0]
	if !val.IsString() {
		return ectx.Arena().WrapError("upper: string arg required", val)
	}
	if val.IsNull() {
		return zed.NullString
	}
...
func (t *Trim) Call(ectx expr.Context, args []zed.Value) zed.Value {
	val := args[0]
	if !val.IsString() {
		return ectx.Arena().WrapError("trim: string arg required", val)
	}
	if val.IsNull() {
		return zed.NullString
	}
...
func (s *Split) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	for i := range args {
		if !args[i].IsString() {
			return ectx.Arena().WrapError("split: string arg required", args[i])
		}
	}
	if sVal.IsNull() || sepVal.IsNull() {
		return ectx.Arena().NewValue(s.typ, nil)
	}
	...
	return ectx.Arena().NewValue(s.typ, b)
}
...
func (j *Join) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	if !ok || typ.Type.ID() != zed.IDString {
		return ectx.Arena().WrapError("join: array of string args required", splitsVal)
	}
	var separator string
	if len(args) == 2 {
		sepVal := args[1]
		if !sepVal.IsString() {
			return ectx.Arena().WrapError("join: separator must be string", sepVal)
		}
...
func (l *Levenshtein) Call(ectx expr.Context, args []zed.Value) zed.Value {
	a, b := args[0], args[1]
	if !a.IsString() {
		return ectx.Arena().WrapError("levenshtein: string args required", a)
	}
	if !b.IsString() {
		return ectx.Arena().WrapError("levenshtein: string args required", b)
	}
