...
func (p *ParseURI) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	if !in.IsString() || in.IsNull() {
		return ectx.Arena().WrapError("parse_uri: non-empty string arg required", in)
	}
	s := zed.DecodeString(in.Bytes())
	u, err := url.Parse(s)
	if err != nil {
		return ectx.Arena().WrapError("parse_uri: "+err.Error(), in)
	}
	var v struct {
	...
	if portString := u.Port(); portString != "" {
		...
		if err != nil {
			return ectx.Arena().WrapError("parse_uri: invalid port: "+portString, in)
		}
		u16 := uint16(u64)
		v.Port = &u16
	}
...
func (p *ParseZSON) Call(ectx expr.Context, args []zed.Value) zed.Value {
	in := args[0]
	if !in.IsString() {
		return ectx.Arena().WrapError("parse_zson: string arg required", in)
	}
	if in.IsNull() {
		return zed.Null
	}
	val, err := zson.ParseValue(ectx.Arena(), zed.DecodeString(in.Bytes()))
	if err != nil {
		return ectx.Arena().WrapError("parse_zson: "+err.Error(), in)
	}
	return val
}
