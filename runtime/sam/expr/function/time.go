...
func (b *Bucket) Call(ectx expr.Context, args []zed.Value) zed.Value {
	...
	if binArg.Type() == zed.TypeDuration {
		...
		if !ok {
			return ectx.Arena().WrapError(b.name+": second argument is not a duration or number", binArg)
		}
	...
	if !ok {
		return ectx.Arena().WrapError(b.name+": first argument is not a time", tsArg)
	}
