...
func (r *recordExpr) Eval(ectx Context, this zed.Value) zed.Value {
	...
	return ectx.Arena().New(r.typ, bytes)
}
...
func (r *recordSpreadExpr) Eval(ectx Context, this zed.Value) zed.Value {
	...
	for _, elem := range r.elems {
		if elem.Spread != nil {
			...
			for _, f := range typ.Fields {
				...
				fv.value = ectx.Arena().New(f.Type, it.Next())
				object[f.Name] = fv
			}
		} else {
	...
	if len(object) == 0 {
		return ectx.Arena().New(r.zctx.MustLookupTypeRecord([]zed.Field{}), []byte{})
	}
	r.update(object)
	b := r.builder
	b.Reset()
	...
	return ectx.Arena().New(r.cache, b.Bytes())
}
...
func (a *ArrayExpr) Eval(ectx Context, this zed.Value) zed.Value {
	...
	if len(a.collection.types) == 0 {
		return ectx.Arena().New(a.zctx.LookupTypeArray(zed.TypeNull), []byte{})
	}
	it := a.collection.iter(a.zctx)
	for !it.done() {
		it.appendNext(&a.builder)
	}
	return ectx.Arena().New(a.zctx.LookupTypeArray(it.typ), a.builder.Bytes())
}
...
func (a *SetExpr) Eval(ectx Context, this zed.Value) zed.Value {
	...
	if len(a.collection.types) == 0 {
		return ectx.Arena().New(a.zctx.LookupTypeSet(zed.TypeNull), []byte{})
	}
	it := a.collection.iter(a.zctx)
	for !it.done() {
		it.appendNext(&a.builder)
	}
	return ectx.Arena().New(a.zctx.LookupTypeSet(it.typ), zed.NormalizeSet(a.builder.Bytes()))
}
...
func (m *MapExpr) Eval(ectx Context, this zed.Value) zed.Value {
	...
	if len(m.keys.types) == 0 {
		...
		return ectx.Arena().New(typ, []byte{})
	}
	m.builder.Reset()
	...
	return ectx.Arena().New(typ, zed.NormalizeMap(bytes))
}
