...
func (s *Shaper) Eval(ectx Context, this zed.Value) zed.Value {
	...
	return ectx.Arena().WrapError("shaper type argument is not a type", typeVal)
}
...
func (c *ConstShaper) Eval(ectx Context, this zed.Value) zed.Value {
	...
	if val.IsNull() {
		...
		return ectx.Arena().NewValue(c.shapeTo, nil)
	}
	id, shapeToID := val.Type().ID(), c.shapeTo.ID()
	if id == shapeToID {
		// Same underlying types but one or both are named.
		return ectx.Arena().NewValue(c.shapeTo, val.Bytes())
	}
	...
	return ectx.Arena().NewValue(typ, c.b.Bytes().Body())
}
...
func (s *step) build(zctx *zed.Context, ectx Context, in zcode.Bytes, b *zcode.Builder) zed.Type {
	...
	case castPrimitive:
		...
		v := s.caster.Eval(ectx, ectx.Arena().NewValue(s.fromType, in))
		b.Append(v.Bytes())
