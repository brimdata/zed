...
var Section = &charm.Spec{
	...
	Short: "extract a section of a zdx file",
	Long: `
...
See the zdx command help for a description of a microindex.`,
	New: newCommand,
}

func init() {
	root.Zdx.Add(Section)
}
...
func (c *Command) Run(args []string) error {
	if len(args) != 1 {
		return errors.New("zdx section: must be run with a single path argument")
	}
	if c.textShortcut {
