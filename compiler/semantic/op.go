...
func maybeConvertCustomOp(ctx context.Context, scope *Scope, call *ast.Call, ds *data.Source, head *lakeparse.Commitish) (dag.Op, error) {
	...
	if call.Where != nil {
		return nil, fmt.Errorf("%s(): custom op cannot have a where clause", call.Name)
	}
	params, args := op.Params, call.Args
	if len(params) != len(args) {
		return nil, fmt.Errorf("%s(): %d arg%s provided when op expected %d arg%s", call.Name, len(params), plural(params), len(args), plural(args))
	}
