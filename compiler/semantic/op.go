...
func (a *analyzer) semOpDeclBody(op *dag.UserOp, params []ast.Param, seq ast.Seq) error {
	...
	var err error
	if op.Body, err = a.semSeq(seq); err != nil {
		return err
	}
	return nil
}
...
// UserOp, otherwise it returns the compile op or the encountered error.
func (a *analyzer) maybeConvertUserOp(call *ast.Call, seq dag.Seq) (dag.Op, error) {
	...
	// The UserOp call needs a Seq but since UserOp's body may not be compiled
	// defer this until the check has completed and we have determined there is
	// not a cycle in op calls.
	a.resolve = append(a.resolve, func() error {
		if op.Body == nil {
			// This should never happen so panic if it does.
			panic("op.Body is nil")
		}
		c.Body = op.Body
		return nil
	})
