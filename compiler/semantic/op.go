...
func (a *analyzer) semSortKeys(sortExprs []ast.SortExpr) order.SortKeys {
	...
	for _, f := range sortExprs {
		s := a.semSortExpr(f)
		if this, ok := s.Key.(*dag.This); !ok {
			// Check not BadExpr so we do not double report errors.
			if _, ok := s.Key.(*dag.BadExpr); !ok {
				a.error(f.Expr, fmt.Errorf("field required in sort expression"))
			}
		} else {
			sortKeys = append(sortKeys, order.NewSortKey(s.Order, this.Path))
		}
	}
	return sortKeys
}
