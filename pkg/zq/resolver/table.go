...
type Table struct {
	mu     sync.RWMutex
	table  []*zq.Descriptor
	lut    map[string]*zq.Descriptor
	caches sync.Pool
}

func NewTable() *Table {
	t := &Table{
		table: make([]*zq.Descriptor, 0),
		lut:   make(map[string]*zq.Descriptor),
	}
...
func (t *Table) UnmarshalJSON(in []byte) error {
	...
	t.lut = make(map[string]*zq.Descriptor)
...
func (t *Table) Lookup(td int) *zq.Descriptor {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if td >= len(t.table) {
		return nil
...
// LookupByValue returns a zq.Descriptor that binds with the indicated
// record type if it exists.  Otherwise, nil is returned.
func (t *Table) LookupByValue(typ *zeek.TypeRecord) *zq.Descriptor {
	t.mu.RLock()
	defer t.mu.RUnlock()
	return t.lut[typ.Key]
}

// GetByValue returns a zq.Descriptor that binds with the indicated
// record type.  If the descriptor doesn't exist, it's created, stored,
// and returned.
func (t *Table) GetByValue(typ *zeek.TypeRecord) *zq.Descriptor {
	key := typ.Key
	t.mu.RLock()
	d := t.lut[key]
	t.mu.RUnlock()
...
func (t *Table) GetByColumns(columns []zeek.Column) *zq.Descriptor {
	typ := zeek.LookupTypeRecord(columns)
	return t.GetByValue(typ)
}

func (t *Table) newDescriptor(typ *zeek.TypeRecord, cols ...zeek.Column) *zq.Descriptor {
...
// AddColumns returns a new zq.Record with columns equal to the given
...
func (t *Table) AddColumns(r *zq.Record, newCols []zeek.Column, vals []zeek.Value) (*zq.Record, error) {
