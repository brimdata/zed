...
func (r *Reader) Read() (*zq.Record, error) {
	for {
...
// ReadPayload returns either data values as zq.Record or control payloads
// as byte slices.  The record and byte slice are volatile so they must be
// copied (via copy for byte slice or zq.Record.Keep()) before any subsequent
// calls to Read or ReadPayload can be made.
func (r *Reader) ReadPayload() (*zq.Record, []byte, error) {
again:
	var hdr header
	err := r.decode(&hdr)
	if err != nil {
...
func (r *Reader) parseValue(id int, b []byte) (*zq.Record, error) {
	descriptor := r.mapper.Map(id)
	if descriptor == nil {
		return nil, zq.ErrDescriptorInvalid
	}
	record := zq.NewVolatileRecord(descriptor, nano.MinTs, b)
	if !record.TypeCheck() {
		return nil, zq.ErrTypeMismatch
	}
