...
func (p *Parser) lookup() (*zq.Descriptor, error) {
...
func (p *Parser) ParseValue(line []byte) (*zq.Record, error) {
	if p.descriptor == nil {
		d, err := p.lookup()
		if err != nil {
			return nil, err
	...
	// each time here
	zv, ts, err := zq.NewRawAndTsFromZeekTSV(p.builder, p.descriptor, []byte(p.path), line)
	if err != nil {
		return nil, err
	}
	return zq.NewRecord(p.descriptor, ts, zv), nil
}
