...
func identity(t *testing.T, logs string) {
	var out Output
	dst := zq.NopFlusher(zqio.NewWriter(&out))
	in := []byte(strings.TrimSpace(logs) + "\n")
	src := zqio.NewReader(bytes.NewReader(in), resolver.NewTable())
	err := zq.Copy(dst, src)
...
// Send logs to zson reader -> bzqio writer -> bzqio reader -> zson writer
func boomerang(t *testing.T, logs string) {
	in := []byte(strings.TrimSpace(logs) + "\n")
	zsonSrc := zqio.NewReader(bytes.NewReader(in), resolver.NewTable())
	var rawZson Output
	rawDst := zq.NopFlusher(bzqio.NewWriter(&rawZson))
	err := zq.Copy(rawDst, zsonSrc)
	require.NoError(t, err)

	var out Output
	rawSrc := bzqio.NewReader(bytes.NewReader(rawZson.Bytes()), resolver.NewTable())
	zsonDst := zq.NopFlusher(zqio.NewWriter(&out))
	err = zq.Copy(zsonDst, rawSrc)
	if assert.NoError(t, err) {
		assert.Equal(t, in, out.Bytes())
	}
}
...
func boomerangZJSON(t *testing.T, logs string) {
	zsonSrc := zqio.NewReader(strings.NewReader(logs), resolver.NewTable())
	var zjsonOutput Output
	zjsonDst := zq.NopFlusher(zjsonio.NewWriter(&zjsonOutput))
	err := zq.Copy(zjsonDst, zsonSrc)
	require.NoError(t, err)

	var out Output
	zjsonSrc := zjsonio.NewReader(bytes.NewReader(zjsonOutput.Bytes()), resolver.NewTable())
	zsonDst := zq.NopFlusher(zqio.NewWriter(&out))
	err = zq.Copy(zsonDst, zjsonSrc)
	if assert.NoError(t, err) {
...
func TestCtrl(t *testing.T) {
	...
	r := zqio.NewReader(bytes.NewReader(in), resolver.NewTable())
