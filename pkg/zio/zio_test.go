...
func TestZsonDescriptors(t *testing.T) {
	...
	r := zqio.NewReader(strings.NewReader(src), resolver.NewTable())
	...
	d := "#1:record[n:int]\n"
	r = zqio.NewReader(strings.NewReader(d+d), resolver.NewTable())
	_, err = r.Read()
